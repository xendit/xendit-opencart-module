...
class ControllerExtensionPaymentXenditBNIVA extends Controller {
    private $error = array();

    const XENDIT_CODE = 'bniva';
    ...
    public function install() {
        if ($this->user->hasPermission('modify', 'extension/extension')) {
            ...
            $this->model_extension_payment_xenditbniva->install();
        }
    }
    ...
    public function uninstall() {
        if ($this->user->hasPermission('modify', 'extension/extension')) {
            $this->load->model('extension/payment/xendit' . self::XENDIT_CODE);
            $this->model_extension_payment_xenditbniva->uninstall();
        }
    }
